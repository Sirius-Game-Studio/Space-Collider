...
public class EnemyHealth : MonoBehaviour
{
    ...
    [Tooltip("Whether if this enemy spawns additional enemies upon death or not.")] [SerializeField] private bool spawnEnemiesOnDeath = false;
    ...
    void Update()
    {
        if (health <= 0)
        {
            ...
            if (spawnEnemiesOnDeath && enemiesToSpawn.Length > 0 && enemyAmount > 0)
            {
                ...
                {
                    Instantiate(enemiesToSpawn[Random.Range(0, enemiesToSpawn.Length)], transform.position + new Vector3(Random.Range(randomSpawnX.x, randomSpawnX.y), 0, Random.Range(randomSpawnZ.x, randomSpawnZ.y)), Quaternion.Euler(-90, 0, 0));
                }
            }
